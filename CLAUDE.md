# CLAUDE.md — Agent Instructions for CVS Blogs Server

This file gives Claude (and any Claude-based agent) full context about this project so it can assist without re-discovering conventions each session.
...
1. **Neon uses a pooler URL** — the DATABASE_URL ends in `-pooler.` — this is correct and intentional. Do not change it to a direct URL for regular operation.
2. **`prisma db push` vs `prisma migrate dev`** — `db push` was used for rapid schema prototyping early on. Switch to `prisma migrate dev` once the schema stabilises to maintain a proper migration history.
3. **`ts-node-dev` uses `--transpile-only`** — TypeScript type errors won't stop the dev server. Always run `npx tsc --noEmit` after changes to validate types before committing.
4. **Express 5** is installed (`^5.2.1`), not Express 4. Async route errors are forwarded automatically in Express 5 without needing `catchAsync`, but we use it anyway for consistency and for Express 4 compatibility if that ever changes.

---

## What's Not Built Yet (Day 1 Carry-overs)

The following are planned but not implemented:

- Authentication (JWT — register, login, protect middleware)
- Post CRUD endpoints
- Tags management
- Comments
...
See `docs/PROGRESS.md` for the full day-by-day development log including decisions, blockers, and carry-overs.
